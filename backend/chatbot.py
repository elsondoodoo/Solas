...
def tell_joke(prompt: str) -> Tuple[FunctionResultStatus, str, dict[str, Any]]:
    ...
    if "sad" in prompt.lower():
        ...
        return (
            ...
            f"Don't be sad, here's a joke to cheer you up: {selected_joke}",
            {},
        )
    else:
        ...
        return (
            FunctionResultStatus.DONE,
            f"It doesn't seem like you're feeling sad, but here's a joke anyway: {selected_joke}",
            {},
        )
...
def stream_music_by_artist(artist: str) -> Tuple[str, str, dict[str, Any]]:
    ...
    try:
        ...
        return (
            FunctionResultStatus.FAILED,
            f"Failed to authenticate with Spotify: {str(e)}",
            {},
        )
    ...
    track_uri = top_track.get("uri") or f"spotify:track:{top_track.get('id')}"
    search_query = urllib.parse.quote(f"{track_name} {artist}")
    ...
    if spotify_search_url:
        message = (
            f"Streaming preview of '{track_name}' by {artist}. "
            f"Listen here: {spotify_search_url}"
        )
        print(f"Streaming preview: {track_name} by {artist} | Preview URL: {track_uri}")
